...
def emulator_snaphot_load(
    ...
    Returns:
        bool: Whether snapshot load was successful (whether ADB connection recovered)
    """
    device = f"emulator-{adb_port}"
    ...
    # Wait for ADB connection to recover (device may briefly disconnect after snapshot load)
    print(f"Waiting for ADB connection to recover after snapshot load...")
    max_wait = 60  # Maximum wait time: 60 seconds
    start_time = time.time()
    while time.time() - start_time < max_wait:
        try:
            result = subprocess.run(
                f"adb -s {device} shell echo ready",
                text=True,
                shell=True,
                capture_output=True,
                timeout=10
            ...
            if result.returncode == 0 and "ready" in result.stdout:
                print(f"ADB connection recovered successfully")
                return True
        except:
            pass
        time.sleep(2)

    print(f"WARNING: ADB connection did not recover after {max_wait} seconds")
    return False
